...
class Range:
    ...
    def track(self, value):
        if value < self.min or self.min == None:
            self.min = value
        if value > self.min or self.max == None:
            self.max = value
...
class Invariants:
    ...
    def track(self, frame, event, arg):
        ...
        if frame.f_code.co_name not in self.vars:
            self.vars[frame.f_code.co_name]={"call":{},"return":{"ret":Range()}}
        if event == "call":
            vars = self.vars[frame.f_code.co_name]["call"]
            for var,val in frame.f_locals.iteritems():
                if var not in vars:
                    vars[var]=Range()
                vars[var].track(val)
            assert vars == self.vars[frame.f_code.co_name]["call"]
        if event == "return":
            vars = self.vars[frame.f_code.co_name]["return"]
            for var,val in frame.f_locals.iteritems():
                if var not in vars:
                    vars[var]=Range()
                vars[var].track(val)
            vars["ret"].track(arg)
            assert vars == self.vars[frame.f_code.co_name]["return"]
